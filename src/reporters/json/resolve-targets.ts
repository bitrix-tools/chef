...
import type { JsonErrorPayload, JsonNotFoundEntry } from './types';
...
/**
 * Validates a TargetSelector. Returns a JsonErrorPayload if `extension` and
 * `path` are both provided (these are mutually exclusive), otherwise null.
 */
export function validateTargetSelector(selector: TargetSelector): JsonErrorPayload | null
{
 ...
 *
 * Names/patterns that match nothing are silently dropped — the result reflects
 * what was actually found. Use the resulting `found.length` if you need to
 * detect "nothing matched" in caller code.
 *
...
export async function resolveTargets(selector: TargetSelector): Promise<ResolveTargetsResult>
{
	...
	if (extension !== undefined)
	{
		...
		return {
			found,
			notFound: notFound.map(({ name, reason }) => ({ name, reason })),
		};
	}
