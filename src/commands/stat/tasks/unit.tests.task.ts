...
export function unitTestsTask(extension: BasePackage): Task
{
	return {
		...
		run: async (context) => {
			...
			{
				context.fail(`Run unit tests failed --> Run chef test -e=${extension.getName()} for more information`);
			}
			else
			{
				...
				if (stats.failed > 0)
				{
					context.fail(`Unit tests failed --> Run chef test -e=${extension.getName()} for more information.`);
				}
			}
		},
	};
