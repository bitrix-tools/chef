...
`chef init build` scans all extensions and generates `aliases.tsconfig.json` with paths so that imports like `import { Tag } from 'main.core'` work in code. See [TypeScript](/en/guide/typescript) for details.

`chef init tests` creates a Playwright config and credentials file for automatic authentication during testing. See [Test Setup](/en/guide/test-setup) for details.
