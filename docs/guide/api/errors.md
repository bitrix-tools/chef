...
| Код                       | Когда возникает                                                                  |
|---------------------------|----------------------------------------------------------------------------------|
| `CF.NOT_FOUND`            | Расширение по указанному имени/паттерну не найдено. Попадает в `result.notFound[]`. |
| `CF.PACKAGE_PROTECTED`    | Расширение помечено `protected` — пропущено.                                     |
| `CF.OPTION_DENIED`        | Несовместимые опции: одновременно `extension` и `path`, либо `--watch` и `--json` в CLI. Попадает в `result.error` (или `throw` в `chef.findPackages`). |
