...
Помимо общих полей (счётчики расширений), `chef.build` добавляет агрегаты по бандлам всего набора:

```ts
type BuildApiResult['summary'] = {
  total: number;        // сколько расширений было собрано
  passed: number;       // сколько собралось без ошибок
  failed: number;       // сколько упало
  durationMs: number;

  // специфика build
  bundlesSize: number;    // суммарный размер всех выходных бандлов в байтах
  warningCount: number;   // суммарно предупреждений по всем расширениям
};
```

`bundlesSize` суммирует `details.bundles[].size` всех `extensions[]`. Удобно для CI-уведомлений вида «общий размер вырос на X KB».
