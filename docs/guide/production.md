...
Chef автоматически заменяет переменные окружения при сборке:
...
При подключении npm-пакетов Chef учитывает [export conditions](https://nodejs.org/api/packages.html#conditional-exports) из `package.json`. В зависимости от режима сборки выбирается соответствующий вариант модуля:

- **Dev** — условия `development`, `browser`, `import`, `default`
- **Production** — условия `production`, `browser`, `import`, `default`

Это позволяет npm-пакетам предоставлять разные сборки для разработки и продакшена. Например, многие библиотеки включают дополнительные проверки и предупреждения в development-сборке, которые отсутствуют в production.
...
Иногда Bitrix-расширение — это обёртка с типами над npm-пакетом. Например, `ui.lexical` предоставляет типы для `lexical`. В standalone-сборке нужен реальный код из npm, а не обёртка. Для этого используется `remap`:
...
export default {
  ...
  standalone: {
    remap: {
      ...
      // Битрикс-расширение → npm-пакет (из node_modules указанного расширения)
      'ui.lexical.core': { npm: 'lexical', from: 'ui.lexical' },

      // Глоб-паттерн → npm-пакеты
...
**Формат записей remap:**

| Формат | Пример | Описание |
|--------|--------|----------|
| `string` | `'ui.forms'` | Заменить на другое Bitrix-расширение |
| `{ npm, from }` | `{ npm: 'lexical', from: 'ui.lexical' }` | Взять npm-пакет из `node_modules` указанного расширения |
| Глоб `*` | `'ui.lexical.*': { npm: '@lexical/*', ... }` | Подстановочный шаблон — `*` заменяется на совпавшую часть |
...
| CSS зависимостей | подключается отдельно | объединяется в один файл |
...
В standalone-режиме Chef автоматически собирает CSS из всех зависимостей, включая CSS-only расширения (без JS). CSS зависимостей объединяется с CSS самого расширения в один файл.

Если зависимости содержат ассеты (картинки, шрифты), они копируются в поддиректории `images/{имя-расширения}/`, чтобы избежать конфликтов имён файлов между расширениями.
...
- **remap** — если зависимость является обёрткой с типами над npm-пакетом, используйте `remap` для подмены на реальный код. Поддерживаются как точные имена, так и глоб-паттерны.
