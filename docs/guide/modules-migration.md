...
# 3. Сгенерировать алиасы путей для TypeScript
chef aliases

# 4. Установить VCS-хуки (автообновление алиасов при pull/merge)
chef init hooks
...
::: danger @bitrix/cli больше не работает
После того как затянете `default`, сборка через `@bitrix/cli` (`bitrix build`) перестанет работать — он не понимает новый формат `.browserslistrc`. Необходимо перейти на `@bitrix/chef`.
:::

## Что меняется

В `default` влита ветка `main/ts`. Изменения затрагивают две области:
