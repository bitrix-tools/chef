...
Rollup + Babel + PostCSS под капотом. TypeScript и Vue 3 SFC из коробки. Watch-режим с горячей перезагрузкой.
...
Большинство команд Chef (`build`, `test`, `lint`) принимают список расширений одинаковым способом.
...
Пакеты собираются последовательно.
