...
Chef — CLI-инструмент для сборки, тестирования и поддержки фронтенд-расширений Bitrix. Он находит `bundle.config.js` или `bundle.config.ts` в структуре проекта и запускает Rollup-сборку для каждого найденного пакета.
...
`chef init build` сканирует все расширения и генерирует `aliases.tsconfig.json` с путями, чтобы в коде работали импорты вида `import { Tag } from 'main.core'`. Подробнее — в разделе [Настройка TypeScript](/ru/guide/typescript).

`chef init tests` создаёт конфиг Playwright и файл с учётными данными для автоматической аутентификации при тестировании. Подробнее — в разделе [Настройка тестов](/ru/guide/test-setup).
...
2. **Сборка бандла** через Rollup:
   - **TypeScript** — компиляция `.ts` файлов
   - **Babel** — транспиляция в целевые браузеры
   - **PostCSS** — автопрефиксы, SVG-оптимизация, инлайн-картинки
   - **Terser** — минификация (если включена)
