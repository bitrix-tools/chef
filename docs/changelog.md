...
## v1.24.2 <Badge type="tip" text="14.09.2026" />
...
## v1.24.1 <Badge type="tip" text="07.09.2026" />

Исправлена сборка TypeScript-компонентов с заданным `namespace`: файл объявлений `script.d.ts`, который создаёт сама сборка, больше не попадает во вход проверки типов. Раньше он переопределял глобальный `BX`, и каждая успешная сборка ломала следующую — появлялись ошибки TS2339 (`BX.PopupWindow`, `BX.Loader`) в файлах, которые никто не менял. По той же причине ложные ошибки выдавал `chef typecheck`. Сторонние файлы объявлений по-прежнему учитываются.

## v1.24.0 <Badge type="tip" text="07.09.2026" />

Путь к странице запуска unit-тестов теперь настраивается через переменную `MOCHA_WRAPPER` в `.env.test`. Раньше адрес `/dev/ui/cli/mocha-wrapper.php` был зашит в код, и на установках, где эта страница доступна по другому адресу, unit-тесты запустить было нельзя. Принимается как путь от корня сайта, так и полный URL — в этом случае хост из `BASE_URL` не используется. Значение по умолчанию не изменилось. В сообщение об ошибке недоступной страницы добавлена подсказка про эту переменную.

## v1.23.1 <Badge type="tip" text="03.09.2026" />

`chef test unit` теперь берёт Playwright из проекта, а не из собственных зависимостей chef.

Раньше unit-тесты импортировали Playwright из node_modules самого chef. При глобальной установке npm разрешает диапазон версий без lock-файла, поэтому копия chef уходила вперёд относительно версии, закреплённой в проекте, и запуск падал с «Executable doesn't exist» на ревизии браузера, которую никто не устанавливал: `npx playwright install` в проекте ставит браузеры только под его собственную версию. При этом `chef test e2e` работал, так как запускает `npx playwright` из корня проекта.

Теперь оба раннера используют одну и ту же копию Playwright и один набор браузеров. Если в проекте Playwright не объявлен, chef, как и раньше, использует свою копию.

## v1.23.0 <Badge type="tip" text="02.09.2026" />

Репортер тестов больше не теряет flaky-тесты: сводка показывает число прошедших с ретрая, называет их по именам и предупреждает, что на упавшей попытке Playwright дописывает отсутствующий эталонный снимок — зелёный прогон с ретраями не доказывает эталон. Прогон, в котором часть отобранных тестов не отчиталась ни одним результатом, теперь падает с сообщением Mismatch вместо зелёного выхода с кодом 0 — в CI такой прогон станет красным. В --reporter json добавлены retries у результата браузера и flaky в счётчики.

## v1.22.0 <Badge type="tip" text="01.09.2026" />
...
## v1.21.0 <Badge type="tip" text="10.07.2026" />

Улучшения вывода chef test:
- Единый статус-бар по браузерам: стадии старта (starting/building/preparing) → счётчики N/M и текущий тест, в одном виде от запуска до финала
- Результаты сгруппированы по describe-блокам (как в --list): путь suite — заголовком, тесты под ним; мульти-браузерный тест — одна строка с тегом всех движков
- Повторно прошедшие тесты (retry) помечаются как flaky, отдельной цифрой в сводке
- Не запускать браузер, если у расширения нет тестовых файлов
- Ctrl+C во время прогона корректно восстанавливает терминал (курсор и режим ввода)

## v1.20.0 <Badge type="tip" text="09.07.2026" />

Улучшения chef test:
- --list — вывод списка тестов без запуска (unit/e2e/модуль), со сводкой по видам
- --console [browser|node|all] — печать вывода console.* из тестов
- Артефакты упавших e2e-тестов (screenshot, video, trace) в отчёте и --json, сгруппированы по браузеру
- JSON-репортер для тестов модуля; единая обработка e2e расширений и модулей
- --grep нормализуется в NFC (корректный матч кириллицы); подсказка, если паттерн из нескольких слов передан без кавычек
- Понятные ошибки вместо «no tests collected»: недоступная тестовая страница, битый конфиг, сбой e2e-прогона

## v1.19.0 <Badge type="tip" text="06.07.2026" />

Убрана поддержка Mercurial. Команды chef flow-to-ts и chef init hooks теперь работают только с Git: переименование файлов через git mv, установка git-хуков вместо .hg/hgrc.

## v1.18.1 <Badge type="tip" text="02.07.2026" />

Исправлены source maps собранных бандлов — позиции больше не съезжают (строки указывают на реальный исходный код). Типы, доступные через контейнер main.core (например Cache.MemoryCache), теперь корректно резолвятся в namespace-ссылку в .d.ts вместо относительного импорта
