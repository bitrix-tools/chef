...
import { resolveTargets, validateTargetSelector } from '../../../src/reporters/json/resolve-targets';
...
describe('resolveTargets', () => {
	...
	it('resolves a single extension by name', async () => {
		const { found, error } = await resolveTargets({ extension: 'ui.buttons' });
	...
	it('returns empty found for non-existent extension', async () => {
		const { found } = await resolveTargets({ extension: 'definitely.nonexistent' });

		assert.lengthOf(found, 0);
	});

	it('keeps the found ones when given a mix', async () => {
